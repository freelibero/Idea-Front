<template>
  <div class="bg-background text-on-surface font-body-md overflow-x-hidden">
    ...
    <section class="relative w-full pt-40 pb-16 bg-surface-dim">
      <div class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
        <div class="flex flex-col items-center text-center max-w-4xl mx-auto md:max-w-5xl">
          <!-- Breadcrumb -->
          <div class="flex items-center gap-4 mb-8 font-label-xs text-label-xs uppercase tracking-[0.2em] text-secondary">
          ...
          <!-- Meta -->
          <div class="flex items-center gap-8 font-label-sm text-label-sm text-on-surface-variant border-y border-midnight-gray py-4 px-8">
    ...
    <!-- Content Layout -->
    <section class="relative max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop flex flex-col lg:flex-row gap-16 pb-24">
      ...
      <article class="flex-1 mx-auto w-full max-w-4xl">
        <div class="font-body-lg text-body-lg text-on-surface-variant space-y-12 leading-[1.8] tracking-wide max-w-none md:text-[22px]">
          ...
          >
            ...
            <p class="relative z-10 font-quote-italic text-quote-italic text-on-surface text-[24px] md:text-[32px] leading-tight">
          ...
          <figure v-if="newsItem?.contentImage" class="my-16">
            <div class="w-full relative overflow-hidden border-[0.5px] border-midnight-gray group">
              <div class="absolute inset-0 bg-black/40 group-hover:bg-transparent transition-colors duration-700 z-10 pointer-events-none"></div>
              <img
                :alt="newsItem.contentImage.caption"
            ...
            <figcaption class="mt-4 font-label-xs text-label-xs text-secondary/80 uppercase tracking-widest flex items-center gap-2">
          ...
          <h2 v-if="newsItem?.contentImage && afterImageParagraphs.length" class="font-headline-md text-headline-md text-on-surface mt-16 mb-6">Elevating the Global Standard</h2>
        ...
        <!-- Tags & Mobile Share -->
        <div class="mt-16 pt-8 border-t-[0.5px] border-midnight-gray flex flex-col md:flex-row justify-between items-center gap-6">
          <div class="flex flex-wrap gap-3">
            <span
              v-for="tag in newsItem?.tags"
              :key="tag"
              class="px-4 py-2 border-[0.5px] border-midnight-gray font-label-xs text-label-xs text-on-surface-variant uppercase tracking-widest hover:border-secondary hover:text-secondary transition-colors cursor-pointer"
            >{{ tag }}</span>
          </div>
        ...
        <div class="mt-24 grid grid-cols-1 md:grid-cols-2 gap-px bg-midnight-gray border-y-[0.5px] border-midnight-gray">
          <!-- Previous -->
          <router-link
            v-if="newsItem?.prev"
            :to="`/news/${newsItem.prev.id}`"
          ...
          >
            <div class="absolute inset-0 bg-linear-to-r from-secondary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            <span class="font-label-xs text-label-xs text-secondary uppercase tracking-widest mb-4 group-hover:-translate-y-1 transition-transform duration-300 flex items-center gap-2">
              <span class="material-symbols-outlined text-[16px]">arrow_back</span> 上一篇
            </span>
            <h3 class="font-headline-md text-[24px] leading-tight text-on-surface-variant group-hover:text-on-surface transition-colors duration-300">
              {{ newsItem.prev.title }}
            </h3>
          </router-link>
          ...
          >
            <div class="absolute inset-0 bg-linear-to-l from-secondary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            <span class="font-label-xs text-label-xs text-secondary uppercase tracking-widest mb-4 group-hover:-translate-y-1 transition-transform duration-300 flex items-center gap-2 md:flex-row-reverse">
      ...
      <div class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-gutter">
          ...
          >
            ...
            <div class="p-8">
              <div class="font-label-xs text-label-xs text-secondary uppercase tracking-widest mb-4">{{ related.tag || 'News' }}</div>
